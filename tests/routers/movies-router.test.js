...
describe('Movies Controller', () => {
  describe('GET /movies', function () {
    context('without query params', () => {
      it('should have status code 200', async function () {
        ///expect.fail('Implement this test'
        const response = await supertest(app).get('/movies').send({}).expect(200)
    ...
    context('with query params', () => {
      context('when limit=10 and offset=10', () => {
        it('should return 10 items', async () => {
          // expect.fail('Implement this test')
          const response = await supertest(app).get('/movies?offset=10&limit=10').send({}).expect(200)
          expect(response.body).to.exist
  ...
  describe('GET /movies/:id', function () {
    context('when correct id is given', () => {
      it('should have status code 200 with the correct movie object', async function () {
        const exsistingId = INITIAL_MOVIES.movies[0].id
        const exsistingMovie = INITIAL_MOVIES.movies[0]
        const request = `movies/${exsistingId}`
        const response = await supertest(app).get(`/movies/${exsistingId}`).expect(200)
        expect(response.body).to.exist
        expect(response.body.id).to.eq(exsistingMovie.id)
        expect(response.body.title).to.eq(exsistingMovie.title)
        expect(response.body.img).to.eq(exsistingMovie.img)
        expect(response.body.synopsis).to.eq(exsistingMovie.synopsis)
        expect(response.body.rating).to.eq(exsistingMovie.rating)
        expect(response.body.year).to.eq(exsistingMovie.year)
    ...
    context('when id of movie doesnt exist', () => {
      it('should return status code 404', async () => {
        // expect.fail('Implement this test')
        await supertest(app).get(`/movies/9999`).expect(404)
  ...
  describe('POST /movies', function () {
    it('should create a new movie object', async function () {
      ...
      const response = await supertest(app).post('/movies').send(newMovieObj).expect(201)
      expect(response.body).to.exist


      //check if the line exsists :

      await supertest(app).get(`/movies/${response.body.id}`).send(newMovieObj).expect(200)
